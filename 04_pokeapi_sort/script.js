...
async function fetchAllPokemonStats() {
  ...
  if (rows.length !== TOTAL_POKEMON * 6) {
    throw new Error("Pokemon stats are incomplete");
  }
  for (const row of rows) {
    const stats = statsCache.get(row.pokemon_id) || {};
    const statName = row.pokemon_v2_stat?.name;
    if (statName) stats[statName] = row.base_stat;
    statsCache.set(row.pokemon_id, stats);
  }
  statsCache.forEach(stats => {
    stats.total = Object.values(stats).reduce((sum, value) => sum + value, 0);
  });
  statsAvailable = statsCache.size === TOTAL_POKEMON;
}
...
function filterCards() {
  ...
  const sortKey = sortStat.value;
  const direction = sortDirection.value === "asc" ? 1 : -1;
  const sortedCards = Array.from(cards).sort((a, b) => {
    const idA = Number(a.dataset.id);
    const idB = Number(b.dataset.id);
    if (!sortKey || !statsAvailable) return idA - idB;
    const difference = statsCache.get(idA)[sortKey] - statsCache.get(idB)[sortKey];
    return difference === 0 ? idA - idB : difference * direction;
  });
...
async function init() {
  ...
  try {
    ...
    await Promise.all([
      ...
      fetchAllPokemonStats().catch(err => {
        sortStat.disabled = true;
        sortStat.title = "能力値を取得できませんでした";
        console.error(err);
      })
    ]);
  } catch (err) {
...
sortStat.addEventListener("change", () => {
  sortDirection.disabled = sortStat.value === "" || !statsAvailable;
  filterCards();
});

sortDirection.addEventListener("change", filterCards);
