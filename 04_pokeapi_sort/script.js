...
const GRAPHQL_API = "https://beta.pokeapi.co/graphql/v1beta";

const grid = document.getElementById("grid");
const countEl = document.getElementById("count");
const searchInput = document.getElementById("search");
const typeFilters = document.getElementById("typeFilters");
const sortStatSelect = document.getElementById("sortStat");
...
async function fetchAllPokemonTypes() {
  ...
  const json = await res.json();
  const rows = json.data?.pokemon_v2_pokemontype ?? [];
  for (let id = 1; id <= TOTAL_POKEMON; id++) {
    typeCache.set(id, []);
  }
  for (const row of rows) {
    const types = typeCache.get(row.pokemon_id) || [];
    const typeName = row.pokemon_v2_type?.name;
    if (typeName) {
      types.push(typeName);
    }
    typeCache.set(
...
async function fetchAllPokemonStats() {
    ...
    query {
      pokemon_v2_pokemonstat(
        where: {
          pokemon_id: { _lte: ${TOTAL_POKEMON} }
          pokemon_v2_stat: {
            name: { _in: ["hp", "attack", "defense", "special-attack", "special-defense", "speed"] }
          }
        }
      ) {
        pokemon_id
  ...
  const rows = json.data?.pokemon_v2_pokemonstat ?? [];
  for (const row of rows) {
    const stats = statsCache.get(row.pokemon_id) || {};
    stats[row.pokemon_v2_stat.name] = row.base_stat;
    statsCache.set(row.pokemon_id, stats);
  }
  for (const stats of statsCache.values()) {
    stats.total = ["hp", "attack", "defense", "special-attack", "special-defense", "speed"]
      .reduce((total, statName) => total + (stats[statName] || 0), 0);
  }
}
...
function buildTypeFilters() {
  ...
  Object.entries(typeNames).forEach(([type, japaneseName]) => {
    const button = document.createElement("button");
    button.className = `type-filter type-${type}`;
    button.dataset.type = type;
    button.textContent = japaneseName;
  ...
  resetButton.className = "type-filter reset-button";
  ...
  typeFilters.appendChild(resetButton);
}

// タイプボタンの選択状態を更新
function updateTypeButtons() {
  const buttons = typeFilters.querySelectorAll(".type-filter");
...
function filterCards() {
  ...
  const sortStat = sortStatSelect.value;
  const sortedCards = Array.from(cards).sort((a, b) => {
    const idA = Number(a.dataset.id);
    const idB = Number(b.dataset.id);
    if (!sortStat) return idA - idB;
    const valueA = statsCache.get(idA)?.[sortStat];
    const valueB = statsCache.get(idB)?.[sortStat];
    if (valueA == null || valueB == null || valueA === valueB) return idA - idB;
  ...
  sortedCards.forEach(card => {
    ...
    const matchesSearch =
      keyword === "" ||
      id === keyword ||
      id.padStart(4, "0") === keyword ||
      name.includes(keyword);
    // タイプによる検索
    // 何も選択していない場合は全タイプを表示
    // 複数選択されている場合は、どれか1つでも一致すれば表示
    const matchesType =
  ...
  countEl.textContent = `${visibleCount}匹`;
}
...
sortStatSelect.addEventListener("change", () => {
  sortDirectionButton.disabled = sortStatSelect.value === "";
  filterCards();
});

sortDirectionButton.addEventListener("click", () => {
  sortDirection = sortDirection === "desc" ? "asc" : "desc";
  sortDirectionButton.textContent = sortDirection === "desc" ? "降順" : "昇順";
  sortDirectionButton.setAttribute("aria-label", `並べ替え方向: ${sortDirectionButton.textContent}`);
  filterCards();
});
